...
class Dataset(Dataset):
    def __init__(self, root_path, test=False):
        ...
        if test:
            ...
            self.images = sorted([root_path+"/train/"+i for i in os.listdir(root_path+"/train/")])[:20]
            self.masks = sorted([root_path+"/train_masks/"+i for i in os.listdir(root_path+"/train_masks/")])[:20]
